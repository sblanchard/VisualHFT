...
public class vmEquityChart : BindableBase
{
    ...
    public List<cEquity> CandlesGrouped { get; private set; }

    public List<cBalance> Balance { get; private set; }

    public List<cDrawDown> DrawDowns { get; private set; }
    ...
    public void LoadData(List<Position> signals)
    {
        ...
        CandlesGrouped = HelperAnalytics.GetEquityCurveByHour(signals);
        Balance = (from x in HelperAnalytics.GetBalanceCurve(signals)
            group x by new { date = new DateTime(x.Date.Year, x.Date.Month, x.Date.Day).AddHours(x.Date.Hour) }
            into g
            select new cBalance
            {
                Date = g.Key.date,
        ...
        DrawDowns = HelperAnalytics.GetDrawdowns(Signals);
