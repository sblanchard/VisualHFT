...
public class vmCharts : BindableBase
{
    ...
    public void LoadData(List<Position> signals)
    {
        ...
        HourSerieWins = (from x in signals.Where(X => X.PipsPnLInCurrency.Value >= 0)
            group x by new
            {
                date = new DateTime(x.CreationTimeStamp.Year, x.CreationTimeStamp.Month, x.CreationTimeStamp.Day,
                    x.CreationTimeStamp.Hour, 0, 0)
            }
            into g
            select new cHourSerie
            {
                Date = g.Key.date.ToDateTime(), PLAmount = g.Sum(s => s.PipsPnLInCurrency.ToDouble()),
                VolumeQty = g.Count()
            }).ToList();
        HourSerieLosses = (from x in signals.Where(X => X.PipsPnLInCurrency.Value < 0)
            ...
            select new cHourSerie
            {
                Date = g.Key.date.ToDateTime(), PLAmount = g.Sum(s => Math.Abs(s.PipsPnLInCurrency.ToDouble())),
                VolumeQty = g.Count()
            }).ToList();
