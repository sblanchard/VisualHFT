...
public class vmChartsStatistics : BindableBase
{
    ...
    public void LoadData(List<Position> signals)
    {
        heldBars = (from x in signals.Where(s => s.CloseTimeStamp != null)
            group x by x.CloseTimeStamp.Subtract(x.CreationTimeStamp).TotalSeconds.ToInt()
            into m
        ...
        var aRanges = new List<double[]>();
        aRanges.Add(new[] { double.MinValue, 0 });
        ...
        aRanges.Add(new[] { 1500, double.MaxValue });
        ...
        PLRangeDuration = signals.OrderBy(x => x.CloseTimeStamp.Subtract(x.CreationTimeStamp).TotalSeconds).Select(x =>
            new cPLRangeDuration(x.CloseTimeStamp.Subtract(x.CreationTimeStamp).TotalSeconds,
                (double)x.PipsPnLInCurrency)).ToList();

        regressionLineData = HelperMath.LinearRegression((from x in signals
            select new PointF((float)x.CloseTimeStamp.Subtract(x.CreationTimeStamp).TotalSeconds,
                (float)x.PipsPnLInCurrency)).ToList());

        MAE = (from x in HelperAnalytics.GetMaximumAdversExcursion(signals)
            select new ScatterChartData(x.Key.ToDouble(), (double)x.Value.GetPipsPnL, "")).ToList();

        regressionLineDataMAE =
            HelperMath.LinearRegression((from x in MAE select new PointF((float)x.XValue, (float)x.YValue)).ToList());

        MFE = (from x in HelperAnalytics.GetMaximumFavorableExcursion(signals)
            select new ScatterChartData(x.Key.ToDouble(), (double)x.Value.GetPipsPnL, "")).ToList();

        regressionLineDataMFE =
            HelperMath.LinearRegression((from x in MFE select new PointF((float)x.XValue, (float)x.YValue)).ToList());
