...
public class AggregatedCollection<T> : IDisposable where T : class, new()
{
    ...
    private readonly AggregationLevel _level;
    private readonly int _maxPoints; // Maximum number of points
    private DateTime lastItemDate = DateTime.MinValue;

    public AggregatedCollection(IEnumerable<T> items, AggregationLevel level, int maxItems,
        Func<T, DateTime> dateSelector, Action<T, T> aggregator)
    {
    ...
    public AggregatedCollection(AggregationLevel level, int maxItems, Func<T, DateTime> dateSelector,
    ...
    public bool Add(T item)
    {
        lock (_lockObject)
        {
            if (_aggregationSpan == TimeSpan.Zero && _level != AggregationLevel.Automatic)
            {
                ...
                {
                    var itemToRemove = _aggregatedData.First();
                    // If 'itemToRemove' is disposable, dispose it before returning to the pool
                    if (_objectPool == null && itemToRemove is IDisposable disposableItem) disposableItem.Dispose();
            ...
            if (_aggregationSpan == GetAggregationSpan(AggregationLevel.Automatic))
            {
                ...
                {
                    bucketTime = new DateTime(itemDate.Ticks / _dynamicAggregationSpan.Ticks *
                                              _dynamicAggregationSpan.Ticks);
                    existingBucket = _aggregatedData.FirstOrDefault(p =>
                        new DateTime(_dateSelector(p).Ticks / _dynamicAggregationSpan.Ticks *
                                     _dynamicAggregationSpan.Ticks) == bucketTime);
                }
            }
            else
            {
                ...
                existingBucket = _aggregatedData.FirstOrDefault(p =>
                    new DateTime(_dateSelector(p).Ticks / _aggregationSpan.Ticks * _aggregationSpan.Ticks) ==
                    bucketTime);
            }
            ...
            {
                //CALL THE AGGREGATOR
                if (_aggregator != null) _aggregator(existingBucket, item);
                return false;
            }
    ...
    public IEnumerator<T> GetEnumerator()
    {
        lock (_lockObject)
        {
            foreach (var item in _aggregatedData) yield return item;
        }
    }
    ...
    public TimeSpan GetAggregationSpan(AggregationLevel level)
    {
        switch (level)
        {
            ...
            case AggregationLevel.Automatic:
                return TimeSpan.Zero; // Default behavior for Automatic. It will be recalculated.
    ...
    private TimeSpan CalculateAutomaticAggregationSpan(DateTime currentItemDate)
    {
        ...
        if (averageElapsed <= TimeSpan.FromMilliseconds(10))
            return GetAggregationSpan(AggregationLevel.Ms10);
        if (averageElapsed <= TimeSpan.FromMilliseconds(100))
            return GetAggregationSpan(AggregationLevel.Ms100);
        if (averageElapsed <= TimeSpan.FromMilliseconds(500))
            return GetAggregationSpan(AggregationLevel.Ms500);
        if (averageElapsed <= TimeSpan.FromSeconds(1))
            return GetAggregationSpan(AggregationLevel.S1);
        if (averageElapsed <= TimeSpan.FromSeconds(3))
            return GetAggregationSpan(AggregationLevel.S3);
        if (averageElapsed <= TimeSpan.FromSeconds(5))
        ...
        if (!_disposed)
        {
            if (disposing)
            {
                if (_objectPool != null)
                {
                    ...
                    foreach (var item in _aggregatedData) _objectPool.Return(item);
                    _aggregatedData.Clear();
                }
