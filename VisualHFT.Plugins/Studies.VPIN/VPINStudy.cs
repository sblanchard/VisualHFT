...
/// <summary>
///     The VPIN (Volume-Synchronized Probability of Informed Trading) value is a measure of the imbalance between buy and
///     sell volumes in a given bucket. It's calculated as the absolute difference between buy and sell volumes divided by
///     the total volume (buy + sell) for that bucket.
///     Given this definition, the range of VPIN values is between 0 and 1:
///     0: This indicates a perfect balance between buy and sell volumes in the bucket. In other words, the number of buy
///     trades is equal to the number of sell trades.
///     1: This indicates a complete imbalance, meaning all the trades in the bucket are either all buys or all sells.
///     Most of the time, the VPIN value will be somewhere between these two extremes, indicating some level of imbalance
///     between buy and sell trades. The closer the VPIN value is to 1, the greater the imbalance, and vice versa.
/// </summary>
public class VPINStudy : BasePluginStudy
{
    ...
    private decimal _currentBuyVolume; // Current volume of buy trades in the bucket
    private decimal _currentSellVolume; // Current volume of sell trades in the bucket
    private bool _disposed; // to track whether the object has been disposed
    private decimal _lastMarketMidPrice; //keep track of market price
    private decimal _lastVPIN;
    ...
    public override string TileToolTip { get; set; } =
        "The <b>VPIN</b> (Volume - Synchronized Probability of Informed Trading) value is a measure of the imbalance between buy and sell volumes in a given bucket.<br/><br/>" +
    ...
    public decimal BucketVolumeSize { get; }
    ...
    private void TRADES_OnDataReceived(Trade e)
    {
        ...
        lock (_locker)
        {
            ...
            if (_currentBuyVolume == 0 && _currentSellVolume == 0) _currentBucketStartTime = e.Timestamp;
            ...
            if (_currentBuyVolume + _currentSellVolume >= BucketVolumeSize)
            {
    ...
    private void LIMITORDERBOOK_OnDataReceived(OrderBook e)
    {
        ...
        lock (_locker)
        {
            if (_orderBook == null) _orderBook = new OrderBook(e.Symbol, e.DecimalPlaces);
    ...
    {
        if (Status != ePluginStatus.STARTED) return;
        decimal vpin = 0;
