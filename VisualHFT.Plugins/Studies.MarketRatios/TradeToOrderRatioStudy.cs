...
///     **Calculation:** It's calculated by dividing the number of executed trades by the total number of orders (including
///     unexecuted ones) in a given time frame.
///     **Usefulness:** A high T2O ratio may indicate strong demand or supply at certain price levels, while a low ratio
///     may suggest indecision or lack of conviction in the market.
/// </summary>
public class TradeToOrderRatioStudy : BasePluginStudy
{
    ...
    private decimal total_L2OrderSize_End;
    private decimal total_L2OrderSize_Ini;
    private decimal totalExecutedTradeSize;
    ...
    public override string TileToolTip { get; set; } =
        "The <b>TTO</b> (Volume - Trade To Order Ratio) value is a key metric that measures the efficiency of trading by comparing the number of executed trades to the number of orders placed.<br/><br/>" +
    ...
    private void LIMITORDERBOOK_OnDataReceived(OrderBook e)
    {
        ...
        if (_settings.Provider.ProviderID != e.ProviderID || _settings.Symbol != e.Symbol)
            return;
        if (_orderBook == null) _orderBook = new OrderBook(_settings.Symbol, e.DecimalPlaces);

        if (!_orderBook.LoadData(e.Asks, e.Bids))
            return; //if nothing to update, then exit

        _lastMarketMidPrice = (decimal)_orderBook.MidPrice;
        var currentOrderSize = e.Asks.Where(x => x.Size.HasValue).Sum(a => (decimal)a.Size) +
                               e.Bids.Where(x => x.Size.HasValue).Sum(b => (decimal)b.Size); // Sum of all order sizes
    ...
    {
        decimal t2oRatio = 0;
        var delta = total_L2OrderSize_End - total_L2OrderSize_Ini;
