...
public class MarketResilienceStudy : BasePluginStudy
{
    ...
    private readonly List<double> _recentSpreads;
    ...
    private double CONDITION3_SPREAD_INCREASE;


    private readonly int LEVELS_TO_CONSUME = 3;
    private readonly double SPREAD_INCREASED_BY = 1.0; //by 100%
    ...
    public override string TileToolTip { get; set; } =
        "<b>Market Resilience</b> (MR) is a real-time metric that quantifies how quickly a market rebounds after experiencing a large trade. <br/> It's an invaluable tool for traders to gauge market stability and sentiment.<br/><br/>" +
    ...
    private void LIMITORDERBOOK_OnDataReceived(OrderBook e)
    {
        ...
        var levelsHasBeenConsumed = false;
        var spreadHasBeenWiden = false;
    ...
    private void CalculateResilience(OrderBook currentOrderBook)
    {
        ...
        {
            var timeSinceLargeTrade = _largeTradeStopwatch.Elapsed;
    ...
    private bool IsDepthRecovered(OrderBook currentOrderBook)
    {
        ...
        var asksHaveRecoveredCount = currentOrderBook.Asks.Count >= LEVELS_TO_CONSUME;
        var bidsHaveRecoveredCount = currentOrderBook.Bids.Count >= LEVELS_TO_CONSUME;

        // Check if the spread has recovered
        var spreadRecovered = currentOrderBook.Spread <= _previousOrderBook.Spread * (1 + SPREAD_INCREASED_BY);

        // Create an instance of BookItem to use its comparison logic
        var comparer = new BookItem();

        // Check if the prices of the levels have recovered
        var asksHaveRecoveredPrice =
            // Scenario 1: Levels have shifted up but count is maintained
            currentOrderBook.Asks.Take(LEVELS_TO_CONSUME)
                .SequenceEqual(_previousOrderBook.Asks.Skip(LEVELS_TO_CONSUME).Take(LEVELS_TO_CONSUME), comparer) ||
            // Scenario 2: Levels have jumped significantly
            !currentOrderBook.Asks.Take(LEVELS_TO_CONSUME)
                .Any(ask => _previousOrderBook.Asks.Any(a => comparer.Equals(a, ask))) ||
            // Scenario 3: Levels have returned to their original state
            currentOrderBook.Asks.Take(LEVELS_TO_CONSUME)
                .SequenceEqual(_previousOrderBook.Asks.Take(LEVELS_TO_CONSUME), comparer);
    ...
    private decimal CalculateResilienceValue(TimeSpan timeSinceLargeTrade, OrderBook currentOrderBook)
    {
        ...
        var maxRecoveryTime = TimeSpan.FromMinutes(1);

        // Calculate normalized time recovery
        var normalizedTimeRecovery = 1 - timeSinceLargeTrade.TotalSeconds / maxRecoveryTime.TotalSeconds;
        ...
        var normalizedSpreadRecovery =
            (_previousOrderBook.Spread - currentOrderBook.Spread) / _previousOrderBook.Spread;
        var normalizedDepthRecovery = (double)(currentOrderBook.Asks.Count + currentOrderBook.Bids.Count) /
                                      (_previousOrderBook.Asks.Count + _previousOrderBook.Bids.Count);

        // Weighted average of the metrics
        var weightTime = 0.7; // Assign a weight to time recovery (can be adjusted)
        var weightDepth = 0.3; // Assign a weight to depth recovery (can be adjusted)

        var resilienceValue = (decimal)(weightTime * normalizedTimeRecovery + weightDepth * normalizedDepthRecovery);
    ...
    {
        if (_previousOrderBook == null) return false;

        var consumedAskLevels = _previousOrderBook.Asks
            .Take(LEVELS_TO_CONSUME)
            .Count(ask => !currentOrderBook.Asks.Any(a => a.Price == ask.Price));

        var consumedBidLevels = _previousOrderBook.Bids
            .Take(LEVELS_TO_CONSUME)
        ...
        if (consumedBidLevels >= LEVELS_TO_CONSUME)
        {
    ...
    private bool PostTradeLOBSpreadIsIncreased(OrderBook currentOrderBook)
    {
        ...
        var spreadCondition = currentOrderBook.Spread > averageSpread * (1 + SPREAD_INCREASED_BY);
