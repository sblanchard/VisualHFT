...
public class MarketResilienceBiasStudy : BasePluginStudy
{
    ...
    public override string TileToolTip { get; set; } =
        "<b>Market Resilience Bias</b> (MRB) is a real-time metric that quantifies the directional tendency of the market following a large trade. <br/> It provides insights into the prevailing sentiment among market participants, enhancing traders' understanding of market dynamics.<br/><br/>" +
    ...
    {
        var wereLevelsReplenishedOnBid = WereLevelsReplenishedOnBid(currentOrderBook);
    ...
    {
        var newBidLevels = currentOrderBook.Bids.Count(bid => !_previousOrderBook.Bids.Any(b => b.Price == bid.Price));
        var newAskLevels = currentOrderBook.Asks.Count(ask => !_previousOrderBook.Asks.Any(a => a.Price == ask.Price));
