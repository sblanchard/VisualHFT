...
public class HelperYahoo
{
    public static YahooStock GetStock(string symbol)
    {
        ...
        //n -> name
        var baseURL = "http://finance.yahoo.com/d/quotes.csv?s={0}&f=npl1vab";
        var url = string.Format(baseURL, symbol);
        ...
        try
        {
            ...
            using (var stReader = new StreamReader(response.GetResponseStream()))
            {
                var output = stReader.ReadLine();
