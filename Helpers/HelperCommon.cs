...
public class HelperCommon
{
    ...
    public static HelperExposure EXPOSURES = new();
    public static HelperActiveOrder ACTIVEORDERS = new();
    public static HelperStrategy ACTIVESTRATEGIES = new();
    public static HelperStrategyParams STRATEGYPARAMS = new();


    public static Dictionary<string, Func<string, string, bool>> GLOBAL_DIALOGS = new()
    {
        { "popup", GetPopup() },
    ...
    public static DateTime GetCurrentSessionIni(DateTime? currentSessionDate)
    {
        ...
        var DateAt00 = currentSessionDate.ToDate();
        var DateAt500PM = DateAt00.AddHours(17);
        var DateAt530PM = DateAt00.AddHours(17).AddMinutes(30);
        if (currentSessionDate > DateAt500PM && currentSessionDate < DateAt00.AddDays(1))
            return DateAt530PM;
        if (currentSessionDate > DateAt00 && currentSessionDate < DateAt500PM)
    ...
    {
        return (msg, capt) => MessageBox.Show(msg, capt, MessageBoxButton.OK, MessageBoxImage.Information) ==
                              MessageBoxResult.OK;
    }

    public static Func<string, string, bool> GetConfirmPopup()
    {
        return (msg, capt) => MessageBox.Show(msg, capt, MessageBoxButton.YesNo, MessageBoxImage.Question) ==
    ...
    {
        return (msg, capt) =>
            MessageBox.Show(msg, capt, MessageBoxButton.OK, MessageBoxImage.Asterisk) == MessageBoxResult.Yes;
    }

    public static Func<string, string, bool> GetErrorPopup()
    {
        return (msg, capt) =>
            MessageBox.Show(msg, capt, MessageBoxButton.OK, MessageBoxImage.Error) == MessageBoxResult.Yes;
    }

    public static void CreateCommonPopUpWindow(FrameworkElement ctrlToSendIntoNewWindow, Button butPopUp,
        object dataContext, string titleWindow = "New Window", int widthWindow = 800, int heightWindow = 600,
        ResizeMode resizeMode = ResizeMode.CanResize)
    {
        ...
        var parent = (Grid)ctrlToSendIntoNewWindow.Parent;
