...
public class HelperPositionAnalysis
{
    ...
    public static List<cEquity> GetEquityCurveByHour(List<Position> aPositions)
    {
        ...
        var hourlyPL = from c in curve.OrderBy(x => x.Date)
            group c by new { date = new DateTime(c.Date.Year, c.Date.Month, c.Date.Day).AddHours(c.Date.Hour) }
            into g
            select new cEquity
            {
                Date = g.Key.date,
    ...
    {
        var aEquity = GetEquityCurve(aSignal);
        var hourly = from x in aEquity.OrderBy(x => x.Date)
            group x by new { date = new DateTime(x.Date.Year, x.Date.Month, x.Date.Day) }
            into g
            select new cEquity
            {
                Date = g.Key.date,
    ...
    public static List<cBalance> GetBalanceCurve(List<Position> aPositions)
    {
        ...
        foreach (var p in aPositions.Where(x => x.PipsPnLInCurrency.HasValue).OrderBy(x => x.CreationTimeStamp))
        {
            ...
            if (p.CloseTimeStamp > p.CreationTimeStamp)
                dRet.Add(new cBalance
                    { Date = p.CloseTimeStamp, Balance = -(p.GetCloseAvgPrice * p.GetCloseQuantity) });
        }
